...
import { Plus, Trash2, Link as LinkIcon, Bold, Italic, List, Link2, Download, Eye } from "lucide-react";
import { currentTheme } from "../../theme/theme";
import { Button, FormInput, FormTextarea, Modal } from "../../components/ui";
...
export default function CVBuilderPage() {
    ...
    const renderCVTemplate = (isPrintView = false) => (
        <div 
            id={isPrintView ? "cv-print-view" : undefined}
            className={`bg-white text-black max-w-[210mm] mx-auto min-h-[297mm] ${isPrintView ? 'hidden print:block p-8' : 'p-4 sm:p-8 shadow-sm border border-gray-200 rounded-lg'}`}
        >
            {/* Header */}
            <div className="flex gap-6 mb-6">
                <div className="w-[110px] h-[130px] shrink-0 bg-gray-100">
                    <img src="/logo_upnvjt.png" alt="Profile" className="w-full h-full object-cover grayscale" />
                </div>
                <div className="flex-1">
                    <h1 className="text-3xl font-extrabold uppercase tracking-wide mb-1 text-gray-900">{details.fullName || "NAMA LENGKAP"}</h1>
                    <div className="text-sm font-medium text-gray-600 mb-1 flex flex-wrap gap-1 items-center">
                        {details.email && <span>{details.email}</span>}
                        {socials.filter(s => s.url).map((s, i) => (
                            <span key={i}> | {s.url}</span>
                        ))}
                    </div>
                    {details.address && <div className="text-sm text-gray-600 mb-2">{details.address}</div>}
                    
                    {details.bio && <div className="text-sm leading-relaxed mt-2 text-gray-800">{details.bio}</div>}
                </div>
            </div>
            ...
            {experiences.some(e => e.company || e.role) && (
                <div className="mb-6">
                    <h2 className="text-lg font-bold mb-3 border-b-2 border-gray-900 pb-1 uppercase text-gray-900">Work Experiences</h2>
                    {experiences.filter(e => e.company || e.role).map((exp, i) => (
                        <div key={i} className="mb-4">
                            <div className="flex justify-between items-baseline mb-1">
                                ...
                                <span className="text-sm text-gray-700 font-medium">{exp.year}</span>
                            </div>
                            <div className="italic text-sm text-gray-700 mb-1">{exp.role}</div>
                            <div className="text-sm text-gray-800 whitespace-pre-wrap leading-relaxed">{exp.description}</div>
                        </div>
                    ))}
                </div>
            )}
            ...
            {educations.some(e => e.institution || e.degree) && (
                <div className="mb-6">
                    <h2 className="text-lg font-bold mb-3 border-b-2 border-gray-900 pb-1 uppercase text-gray-900">Education</h2>
                    {educations.filter(e => e.institution || e.degree).map((edu, i) => (
                        <div key={i} className="mb-4">
                            <div className="flex justify-between items-baseline mb-1">
                                <h3 className="font-bold text-gray-900 text-base">{edu.institution}</h3>
                                <span className="text-sm text-gray-700 font-medium">{edu.year}</span>
                            </div>
            ...
            {/* Organizations */}
            {organizations.some(o => o.name || o.role) && (
                <div className="mb-6">
                    <h2 className="text-lg font-bold mb-3 border-b-2 border-gray-900 pb-1 uppercase text-gray-900">Organizations</h2>
                    {organizations.filter(o => o.name || o.role).map((org, i) => (
                        <div key={i} className="mb-4">
                            <div className="flex justify-between items-baseline mb-1">
                                <h3 className="font-bold text-gray-900 text-base">{org.name}</h3>
                                <span className="text-sm text-gray-700 font-medium">{org.year}</span>
                            </div>
                            <div className="italic text-sm text-gray-700 mb-1">{org.role}</div>
                            <div className="text-sm text-gray-800 whitespace-pre-wrap leading-relaxed">{org.description}</div>
                        </div>
                    ))}
                </div>
            )}
            ...
            {/* Projects */}
            {projects.some(p => p.name || p.role) && (
                <div className="mb-6">
                    <h2 className="text-lg font-bold mb-3 border-b-2 border-gray-900 pb-1 uppercase text-gray-900">Projects</h2>
                    {projects.filter(p => p.name || p.role).map((proj, i) => (
                        <div key={i} className="mb-4">
                            <div className="flex justify-between items-baseline mb-1">
                                <h3 className="font-bold text-gray-900 text-base">{proj.name}</h3>
                                <span className="text-sm text-gray-700 font-medium">{proj.year}</span>
                            </div>
                            <div className="italic text-sm text-gray-700 mb-1">{proj.role}</div>
                            <div className="text-sm text-gray-800 whitespace-pre-wrap leading-relaxed">{proj.description}</div>
                        </div>
                    ))}
                </div>
            )}
            ...
            {skills && (
                <div className="mb-6">
                    <h2 className="text-lg font-bold mb-3 border-b-2 border-gray-900 pb-1 uppercase text-gray-900">Skills & Certifications</h2>
                    <div className="text-sm text-gray-800 whitespace-pre-wrap leading-relaxed">{skills}</div>
                </div>
            )}
        </div>
    );
    ...
    return (
        <>
            <div className="max-w-4xl mx-auto space-y-8 pb-12 print:hidden">
                {/* Top Header area */}
                ...
                <div className="flex items-center justify-end gap-4 pt-4">
                    <Button variant="outline" className="px-6 py-3" onClick={() => setShowPreview(true)} icon={<Eye size={18} />}>
                        Pratinjau CV
                    </Button>
                    <Button variant="primary" icon={<Download size={18} />} className="px-6 py-3" onClick={handlePrint}>
                        Simpan & Download CV
                    </Button>
                </div>
                ...
                <Modal
                    ...
                    footer={
                        <>
                            ...
                            <Button variant="primary" icon={<Download size={18} />} onClick={handlePrint}>Download PDF</Button>
                        </>
                    }
                >
                    <div className="bg-gray-50 rounded-xl flex items-center justify-center p-4">
                        {renderCVTemplate(false)}
                    </div>
                </Modal>
            </div>

            {/* Hidden Print Version (Placed outside the print:hidden container) */}
            {renderCVTemplate(true)}
        </>
    );
}
